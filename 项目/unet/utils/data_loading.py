...
#print('lengt:::',length)
        ...
        img_ndarray = np.array(pil_img, dtype=int)

        if is_mask:
            if len(img_ndarray.shape) == 3:
                img_ndarray = img_ndarray[:, :, 0]
            img_ndarray[img_ndarray <= 127] = 0
            img_ndarray[img_ndarray > 127] = 1
    ...
    def load(filename):
        ...
        if ext == '.npy':
            tmp = Image.fromarray(np.load(filename))
            if(tmp.mode!='RGB'):
                tmp = tmp.convert("RGB")
            return tmp
        elif ext in ['.pt', '.pth']:
            tmp = Image.fromarray(torch.load(filename).numpy())
            if(tmp.mode!='RGB'):
                tmp = tmp.convert("RGB")
            return tmp
        else:
